<template>
  <div class="login-wrap">
    <el-form label-position="left" label-width="0px" class="demo-ruleForm login-container">
      ...
      <el-form-item>
      <el-input type="text" v-model="userName" clearable placeholder="请输入账号名称"></el-input>
      </el-form-item>
      <el-form-item>
        <el-input type="text" style="width:60%;" v-model="phoneNumber" clearable placeholder="请输入电话号码"
                  autocomplete="off"
        ></el-input>
        <el-button type="primary" style="width:38%;" @click="getValideCode">{{valideText}}</el-button>
      </el-form-item>
      <el-form-item>
        <el-input type="text" style="width:60%;" v-model="phoneCode" clearable placeholder="请输入获取的手机验证码"
                  autocomplete="off"
        ></el-input>
      </el-form-item>
      <el-form-item>
        <el-input type="text" style="width:50%;" v-model="jpgVerify" clearable placeholder="请输入验证码" autocomplete="off"
        ></el-input>
        <el-link type="success" @click="changeCount()" style="color: darkblue;" class="myLink02">
          {{identifyCode}}
        </el-link>&emsp;
        <el-button style="width:25%;" @click="changeCount()">点击切换</el-button>
      </el-form-item>
      <el-form-item>
        <el-button type="primary" style="width: 100%;" @click="doSubmit">验证</el-button>
      </el-form-item>
      <el-form-item style="text-align: center;">
        <el-button type="default" style="width: 100%;" @click="back">返回</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>
    ...
    methods: {
      ...
      doSubmit: function () {
        ...
        if (this.f1 == true) {
          ...
          if (this.f2 == true) {
            ...
            //后台请求
            let url = this.axios.urls.sys_user_selectSysUserSname;
            this.axios.post(url, form).then((resp) => {
              if (0 == resp.data.code) {
                ...
                this.$store.commit('setTempName', {
                  tempName: resp.data.result.userName
                });
                //给vuex中userid赋值
      ...
      getValideCode: function () {
        ...
        if (!reg.test(this.phoneNumber)) {
          ...
          //请求地址区域
          let url = this.axios.urls.sys_user_sendSmsSysUser;
          this.axios.post(url, {
