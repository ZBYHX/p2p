<template>
  <div class="login-wrap">
    <el-form label-position="left" label-width="0px" class="demo-ruleForm login-container">
      ...
      <el-form-item>
        <el-input type="text" v-model="userName" readonly="readonly" placeholder="请输入账号" clearable></el-input>
      </el-form-item>
      <el-form-item>
        <el-input type="password" v-model="userPassword" placeholder="请输入密码" clearable autocomplete="off" show-password></el-input>
      </el-form-item>
      <el-form-item>
        <el-input type="password" v-model="repassword" placeholder="请输入确认输入" clearable autocomplete="off" show-password></el-input>
      </el-form-item>
      <el-form-item>
  ...
  export default {
    ...
    data: function() {
      return {
        ...
        userId: this.$store.getters.tempUserid,//用户id
        userPassword: null,//用户新密码
        repassword: null,//确认密码
        f1: false //判断文本验证是否完全正确
      }
    },
    methods: {
      //表单提交
      doSubmit: function() {
        ...
        if(this.f1 == true){
          ...
          //后台请求
          let url = this.axios.urls.sys_user_editStaffPwd;
